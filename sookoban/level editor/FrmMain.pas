...
2:paintbox1.canvas.copyrect(rect((tel2-1)*40,(tel1-1)*40,
   ((tel2-1)*40)+40,((tel1-1)*40)+40),start.canvas,rect(0,0,39,39));
...
opendialog1.InitialDir := settingsfile.readstring('Sookoban level editor','openpath','c:\');
savedialog1.initialdir := settingsfile.readstring('Sookoban level editor','savepath','c:\');
settingsfile.free;
...
if nothing > 0 then
begin
 application.messagebox('Error: There are still empty spaces!','Save Error',MB_OK);
 maysave := false;
end;
if start <> 1 then
begin
 application.messagebox('Error: No start position set or too many set!','Save Error',MB_OK);
 maysave := false;
end;
...
settingsfile.writestring('Sookoban level editor','openpath',opendialog1.InitialDir);
settingsfile.writestring('Sookoban level editor','savepath',savedialog1.initialdir);
settingsfile.free;
end;
