...
type
  ...
  public
    ...
    function CheckField(X: integer; Y: integer; NRonRow: integer; player:
      integer): boolean;
...
begin
  ...
  for y := 0 to NrVertical - 1 do
    for x := 0 to NrHorizontal - 1 do
      if checkfield(x, y, 4, 1) or checkfield(x, y, 4, 2) then
        buf := true;
  is4onarow := buf;
end;
...
function Tform1.CheckField(X: integer; Y: integer; NRonRow: integer; player:
  integer): boolean;
var
...
function Tform1.Setpiece(column: integer; sort: integer; draw: boolean):
  integer;
var
  pos: integer;
begin
...
begin
  Boardimg.canvas.copyrect(rect(0, 0, 410, 385), BoardBackupImg.canvas, rect(0,
    0, 410, 385));
end;
...
begin
  case player of
    1: boardimg.Canvas.copyrect(rect(27 + (x * 50), 33 + (y * 50), 27 + ((x + 1)
      * 50), 33 + ((y + 1) * 50)), ButtonRedImg.canvas, rect(0, 0, 50, 50));
    2: boardimg.Canvas.copyrect(rect(27 + (x * 50), 33 + (y * 50), 27 + ((x + 1)
      * 50), 33 + ((y + 1) * 50)), ButtonBlueImg.canvas, rect(0, 0, 50, 50));
  end;
end;
...
begin
  if gamestarted then
  begin
    if application.messagebox('You are already playing a game,' + chr(13) +
      'Are you sure you want to start a new game and stop the current one?',
...
begin
  ...
  begin
    if application.messagebox('You are already playing a game,' + chr(13) +
      'setting player 2 to PC Player will restart the current game' + chr(13) +
      'Are you sure you want to set player 2 to PC Player and start a new game?',
      'Confirmation', MB_YESNO + MB_ICONQUESTION) = IDYES then
    begin
...
begin
  ...
  begin
    if application.messagebox('You are already playing a game,' + chr(13) +
      'setting player 2 to Human Player will restart the current game' + chr(13) +
      'Are you sure you want to set player 2 to Human Player and start a new game?',
      'Confirmation', MB_YESNO + MB_ICONQUESTION) = IDYES then
    begin
