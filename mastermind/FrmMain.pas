...
begin
  ...
  for Counter := 0 to 3 do
    if playfield[GuessNr, Counter] = empty then
      Value := False;
...
begin
  for Counter := 0 to 3 do
    if playfield[GuessNr, Counter] = Playfield[10, Counter] then
      temp[counter] := true
    else
      temp[Counter] := false;
...
begin
  Playfieldimage.Canvas.CopyRect(rect(0, 0, 199, 399),
    BackupPlayFieldImage.Canvas, rect(0, 0, 199, 399));
end;
...
begin
  ...
  begin
    case Playfield[10, Counter] of
      Black:
        begin
          PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 18, 45 + (30
            * counter), 47), BlackButtonImage.Canvas, rect(0, 0, 29, 29));
        end;
      White:
        begin
          PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 18, 45 + (30
            * counter), 47), WhiteButtonImage.Canvas, rect(0, 0, 29, 29));
        end;
      Blue:
        begin
          PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 18, 45 + (30
            * counter), 47), BlueButtonImage.Canvas, rect(0, 0, 29, 29));
        end;
      Red:
        begin
          PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 18, 45 + (30
            * counter), 47), RedButtonImage.Canvas, rect(0, 0, 29, 29));
        end;
      LightBlue:
        begin
          PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 18, 45 + (30
            * counter), 47), LightBlueButtonImage.Canvas, rect(0, 0, 29, 29));
        end;
      Green:
        begin
          PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 18, 45 + (30
            * counter), 47), GreenButtonImage.Canvas, rect(0, 0, 29, 29));
        end;
      Purple:
        begin
          PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 18, 45 + (30
            * counter), 47), PurpleButtonImage.Canvas, rect(0, 0, 29, 29));
        end;
      Yellow:
        begin
          PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 18, 45 + (30
            * counter), 47), YellowButtonImage.Canvas, rect(0, 0, 29, 29));
        end;
    end;
  end;
end;
...
begin
  ...
  if soundenabled then
    sndplaysound('restart.wav', snd_nodefault + snd_async);
end;
...
begin
  ...
  begin
    for counter := 0 to 3 do
    begin
      if (X >= 16 + (30 * counter)) and (X <= 45 + (30 * counter)) and (Y >= 350
        - (32 * GuessNr)) and (Y <= 379 - (32 * GuessNr)) then
      begin
        case Selectedbutton of
          Black:
            begin
              PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 350 - (32
                * Guessnr), 45 + (30 * counter), 379 - (32 * guessnr)),
                BlackButtonImage.Canvas, rect(0, 0, 29, 29));
          ...
          White:
            begin
              PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 350 - (32
                * Guessnr), 45 + (30 * counter), 379 - (32 * guessnr)),
                WhiteButtonImage.Canvas, rect(0, 0, 29, 29));
          ...
          Blue:
            begin
              PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 350 - (32
                * Guessnr), 45 + (30 * counter), 379 - (32 * guessnr)),
                BlueButtonImage.Canvas, rect(0, 0, 29, 29));
          ...
          Red:
            begin
              PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 350 - (32
                * Guessnr), 45 + (30 * counter), 379 - (32 * guessnr)),
                RedButtonImage.Canvas, rect(0, 0, 29, 29));
          ...
          LightBlue:
            begin
              PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 350 - (32
                * Guessnr), 45 + (30 * counter), 379 - (32 * guessnr)),
                LightBlueButtonImage.Canvas, rect(0, 0, 29, 29));
          ...
          Green:
            begin
              PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 350 - (32
                * Guessnr), 45 + (30 * counter), 379 - (32 * guessnr)),
                GreenButtonImage.Canvas, rect(0, 0, 29, 29));
          ...
          Purple:
            begin
              PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 350 - (32
                * Guessnr), 45 + (30 * counter), 379 - (32 * guessnr)),
                PurpleButtonImage.Canvas, rect(0, 0, 29, 29));
          ...
          Yellow:
            begin
              PlayFieldImage.Canvas.CopyRect(rect(16 + (30 * counter), 350 - (32
                * Guessnr), 45 + (30 * counter), 379 - (32 * guessnr)),
                YellowButtonImage.Canvas, rect(0, 0, 29, 29));
              PlayField[GuessNr, Counter] := Yellow;
            end;
        end;
        if soundenabled then
          sndplaysound('blip1.wav', snd_nodefault + snd_async);
      end;
    end;
  end;
end;
  ...
  for counter1 := 0 to 1 do
    ...
    begin
      case Smallbuttons[Guessnr, counter1 + counter2 + (1 * counter1)] of
        SmallBlack:
          begin
            Playfieldimage.Canvas.CopyRect(rect(153 + (counter1 * 15), 350 +
              (counter2 * 14) - guessnr * 32, 167 + (counter1 * 15), 364 +
              (counter2 * 15) - guessnr * 32), SmallbuttonBlackImage.canvas,
              rect(0, 0, 14, 14));
          end;
        SmallWhite:
          begin
            Playfieldimage.Canvas.CopyRect(rect(153 + (counter1 * 15), 350 +
              (counter2 * 14) - guessnr * 32, 167 + (counter1 * 15), 364 +
              (counter2 * 15) - guessnr * 32), SmallbuttonWhiteImage.canvas,
              rect(0, 0, 14, 14));
          end;
      end;
    end;
  ...
  begin
    if guesscode[counter1] <> empty then
      for Counter2 := 0 to 3 do
        if (Guesscode[Counter1] = CodeToFind[Counter2]) and
          (smallbuttons[guessnr, Counter2] <> SmallBlack) then
        begin
...
begin
  if messagedlg('I noticed this is the first time you run Master Mind,' + chr(13)
    + 'so we will set some settings (wich you can also change later).' + chr(13) +
    'Do you want to enable possible double colors in the color code to find ?',
  ...
  if messagedlg('One last question,' + chr(13) +
    'Do you want to enable sound ?', mtConfirmation, [MByes, Mbno], -1) = IDyes
    then
  begin
    soundenabled := true;
...
begin
  ...
  begin
    ...
    SelectedButton := Black;
    if soundenabled then
      sndplaysound('blip2.wav', snd_nodefault + snd_async);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      drawcomputerButtons;
      if soundenabled then
        sndplaysound('winner.wav', snd_nodefault + snd_async);
    end;
    GuessNr := GuessNr + 1;
    ...
    begin
      ...
      drawcomputerButtons;
      if soundenabled then
        sndplaysound('Loser.wav', snd_nodefault + snd_async);
    end;
    if gamestarted and soundenabled then
      sndplaysound('blip3.wav', snd_nodefault + snd_async);
    Sethandposition;
  end
  else
    messagedlg('You have to set Four Colored pegs on' + chr(13) +
      'the board in order to end your Guess!', mtConfirmation, [MbOk], -1);
end;

procedure TForm1.FormCreate(Sender: TObject);
begin
  form1.doublebuffered := true;
  if not exist('./MasterMind.ini') then
    dofirstrun
  else
    loadsettings;
  if soundenabled then
    sndplaysound('dummy.wav', snd_nodefault + snd_async);
  InitializeGame;
end;
...
begin
  Savesettings;
  if soundenabled then
    sndplaysound('goodbye.wav', snd_nodefault + SND_SYNC);
end;

procedure TForm1.AllowDoubles1Click(Sender: TObject);
begin
  if
    messagedlg('You can Only change the allow doubles setting if you start a new game!'
    + chr(13) +
    'Do you want to change the allow doubles setting and start a new game ?',
    Mtwarning, [MbYes, MbNo], -1) = IDYes then
  begin
...
begin
  ...
  if soundenabled then
    sndplaysound('dummy.wav', snd_nodefault + snd_async);
      //needed to get rid of small sound delay
end;
